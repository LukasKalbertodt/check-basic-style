...
const main = async () => {
    ...
    for (const pattern of config.files) {
        glob.sync(pattern, { nodir: true }).forEach(m => {
            ...
            } else if (isBinaryFile(m)) {
                core.debug(`Will ignore binary file: ${m}`);
            } else {
                paths.add(m);
            }
        });
...
/** Some binary file endings of formats that can be found in git repositories. */
const BINARY_FILE_ENDINGS = [
    ".pdf",
    ".woff2",
    ".woff",
    ".ttf",
...
const isBinaryFile = (filename: string): boolean => BINARY_FILE_ENDINGS
    .some(ending => filename.slice(-ending.length)
        .localeCompare(ending, undefined, { sensitivity: "base" }) === 0
    );
